...
function ExamMarks({ studentId }) {
  ...
  const handleSubmitFilter = async () => {
    try {
      const response = await axios.get(`${BaseUrl}student/Mark/parent`, {
  ...
  return (
    <PageWrapper>
      <div className=" bg-zinc-50 p-4 md:p-6 rounded-xl shadow-md min-h-[80vh] text-white">
        <h2 className="text-2xl font-bold mb-6 text-black flex items-center gap-2 mt-15">
          🎯 Student Marks
        </h2>
              ...
              <tbody>
                {marksData.map((exam, index) =>
                  exam.marks.map((mark, i) => (
                    <tr
                      key={`${index}-${i}`}
                      className="hover:bg-gray-300 transition text-black"
                    >
